...
class MultipleTimer(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.__pref = pref.Preference(self)
    ...
    def __setup_menu_actions(self):
        ...
        __menu_config = QtWidgets.QMenu('Config')
        __menu_help = QtWidgets.QMenu('Help')

        # actions
        self.__action_quit = QtWidgets.QAction('Quit')
        self.__action_pref = QtWidgets.QAction('Preference')
        ...
        self.__action_pref.triggered.connect(lambda: self.__pref.show())
        ...
        self.__action_pref.setShortcut(pref_shortcut_key)

        __menu_file.addAction(self.__action_quit)
        __menu_config.addAction(self.__action_pref)
        __menu_help.addAction(self.__action_about)

        self.__menubar.addMenu(__menu_file)
        self.__menubar.addMenu(__menu_config)
    ...
    def __setup_ui(self):
        work_threads_from_pref = self.__pref.dbl_spinbox_work_threads.value()

        self.__setup_menu_actions()
        label_cnt = QtWidgets.QLabel('Count Work Threads:')
        self.__spinbox_thread_cnt = QtWidgets.QSpinBox()
        self.__spinbox_thread_cnt.setValue(work_threads_from_pref)
    ...
    def __setup_widgets_ui(self):
        ...
        for i in range(cnt_threads):
            ...
            widget.comboBox__dependency.addItems(list(map(lambda x: str(x), range(cnt_threads))))
            widget.comboBox__link.addItems(list(map(lambda x: chr(x + 65), range(cnt_threads))))
            widget.signals.changed_link.connect(self.__slot_changed_link)

            widget.signals.finished_work.connect(self.__slot_finished_work)
            widget.adjustSize()
    ...
    @QtCore.Slot(str, int)
    def __slot_changed_link(self, jid, idx):
        w: singleTimer.SingleTimer = self.__widget_data.get(jid)
        link_val = w.comboBox__link.itemText(idx)
        print(jid, w)
        if link_val not in list(self.__link_data.keys()):
            self.__link_data[link_val] = [w]
        else:
            self.__link_data[link_val].append(w)


    @QtCore.Slot(str)
    def __slot_finished_work(self, jid):
        print(jid, 'finished work!')
