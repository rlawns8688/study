...
class Signals(QtCore.QObject):
    ...
    finished_work = QtCore.Signal(str)
...
class WorkThread(QtCore.QThread):
    ...
    def set_ste_waiting(self) -> None:
        if self.bitfield.confirm(Constant.RUNNING | Constant.WAITING):
            ...
            self.bitfield.deactivate(
                Constant.RUNNING | Constant.STOPPED | Constant.FINISHED
            )
...
SelfSingleTimer = typing.TypeVar("SelfSingleTimer", bound="SingleTimer")
...
class SingleTimer(QtWidgets.QWidget, timer_ui.Ui_Form__timer):
    ...
    def __init__(self, parent=None):
        ...
        self.__subscribers = {self.__jid: self}
        self.__publisher = parent
        ...
        # connections
        self.pushButton__start.clicked.connect(
            lambda: self.slot_timer(SingleTimer.METHOD_START)
        )
        self.pushButton__stop.clicked.connect(
            lambda: self.slot_timer(SingleTimer.METHOD_STOP)
        )
        # added
        self.pushButton__subscribe.pressed.connect(self.slot_subscribe)
        ...
        self.timeEdit__timer.timeChanged.connect(self.slot_time_changed)

    def slot_subscribe(self):
        is_subscribe = not self.pushButton__subscribe.isChecked()
        if not is_subscribe:
            self.__publisher.detach(self.jid)
        else:
            self.__publisher.attach(self)

        self.__publisher.show_subscribers()

    def update_alert(self):
        print(f"alert string: {self.__publisher.alert_str()} >> {self.jid}")
        if self.__publisher.alert_str() == SingleTimer.METHOD_START:
            SingleTimer.start_timer(self)
        else:
            SingleTimer.stop_timer(self)
    ...
    def slot_time_changed(self, time: QtCore.QTime):
        if not len(self.__subscribers):
            return
        for k, v in self.__subscribers.items():
            v: SelfSingleTimer
            if self.__jid == k or self.comboBox__link.currentText() == "null":
                continue
            if self.comboBox__link.currentText() == v.comboBox__link.currentText():
                v.timeEdit__timer.setTime(time)
    ...
    def slot_add_item(self):
        ...
        item.combobox.currentIndexChanged.connect(
            lambda: self.slot_lstwdg_cmb_changed(
                self.listWidget__command.currentIndex()
            )
        )
    ...
    def slot_del_item(self):
        ...
        if not qt_lib.QtLibs.question_dialog(
            "Remove Run Command",
            f"Should I remove the {item.current_text} command?",
            self,
        ):
            return
    ...
    def slot_update_ui(self, data: Data) -> None:
        if self.__work_thread.bitfield.confirm(Constant.STARTED | Constant.RUNNING):
            if self.__work_thread.bitfield.confirm(Constant.STARTED):
                ...
                Color.set_color_progressbar(
                    self.progressBar__remaining, Color.status.get(Constant.RUNNING)
                )
                self.lcdNumber__remaining.display(
                    SingleTimer.sec2qtime(data.sec).toString()
                )
                self.listWidget__command.setEnabled(False)
        elif self.__work_thread.bitfield.confirm(Constant.WAITING):
            Color.set_color_progressbar(
                self.progressBar__remaining, Color.status.get(Constant.WAITING)
            )
        elif self.__work_thread.bitfield.confirm(Constant.STOPPED | Constant.ERROR):
            Color.set_color_progressbar(
                self.progressBar__remaining, Color.status.get(Constant.STOPPED)
            )
            if self.__work_thread.bitfield.confirm(Constant.ERROR):
                Color.set_color_progressbar(
                    self.progressBar__remaining, Color.status.get(Constant.ERROR)
                )
            ...
            self.__init_set()
            Color.set_color_progressbar(
                self.progressBar__remaining, Color.status.get(Constant.FINISHED)
            )
            # emitting finished work signal
            self.signals.finished_work.emit(self.__jid)
            if data.sec <= 0:
                self.run_commands()
        self.label__status.setText(data.msg)
        if self.__work_thread.bitfield.confirm(Constant.ERROR):
    ...
    def get_subscribers(self) -> list:
        ...
        grp_str = self.comboBox__link.currentText()
        return list(
            filter(
                lambda x: x.comboBox__link.currentText() == grp_str, self.__subscribers
            )
        )

    def slot_timer(self, method, is_batch=False):
        for jid, subscriber in self.__subscribers.items():
            subscriber: SelfSingleTimer
            if not is_batch:
                if (
                    self.comboBox__link.currentText()
                    != subscriber.comboBox__link.currentText()
                ):
                    continue
            if method == SingleTimer.METHOD_START:
                SingleTimer.start_timer(subscriber)
            else:
                SingleTimer.stop_timer(subscriber)

    @staticmethod
    def start_timer(subscriber: SelfSingleTimer):
        if not subscriber.is_set_timer():
            QtWidgets.QMessageBox.warning(subscriber, "Warning", "타이머 설정을 해야 합니다.")
            return
        if not subscriber.work_thread.isRunning():
            subscriber.work_thread.set_ste_started()
            # start
            subscriber.work_thread.run_start(
                subscriber.qtime2sec(subscriber.timeEdit__timer.time())
            )
            subscriber.timeEdit__timer.setEnabled(False)
            if subscriber.work_thread.bitfield.confirm(
                Constant.RUNNING | Constant.WAITING
            ):
                subscriber.pushButton__start.setText("Pause")
                if subscriber.work_thread.bitfield.confirm(Constant.WAITING):
                    subscriber.pushButton__start.setText("Start")
        else:
            subscriber.work_thread.set_ste_waiting()
            if not subscriber.work_thread.bitfield.confirm(Constant.WAITING):
                subscriber.work_thread.resume()
                subscriber.pushButton__start.setText("Pause")
            else:
                subscriber.pushButton__start.setText("Resume")

    @staticmethod
    def stop_timer(subscriber: SelfSingleTimer):
        if subscriber.work_thread.isRunning():
            subscriber.work_thread.stop()
